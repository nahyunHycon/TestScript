...
echo "$(date '+%Y-%m-%d %H:%M:%S') - [2.1] Ros2_ID Test started" >> "$LOG_FILE"
...
if [ "$extracted_value" == "$EXISTING_VALUE" ]; then
  echo "$(date '+%Y-%m-%d %H:%M:%S') - [2.1] Ros2_ID Test Test result = 0 " >> "$LOG_FILE"
  echo "0"  
else
  echo "$(date '+%Y-%m-%d %H:%M:%S') - [2.1] Ros2_ID Test Test result = 1 " >> "$LOG_FILE"
  echo "1"  
fi

sleep 1
echo "$(date '+%Y-%m-%d %H:%M:%S') - [2.1] Ros2_ID Test finished" >> "$LOG_FILE"
