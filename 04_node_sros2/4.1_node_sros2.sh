...
echo "$(date '+%Y-%m-%d %H:%M:%S') - [4.1] Node_sros2 Test started" >> "$LOG_FILE"
...
if [ "$existing_value1" == "$extracted_value1" ]&&[ "$existing_value2" == "$extracted_value2" ]&&[ "$existing_value3" == "$extracted_value3" ]; then
  echo "$(date '+%Y-%m-%d %H:%M:%S') - [4.1] Node_sros2 Test result = 0" >> "$LOG_FILE" 
  echo "0"  
else
  echo "$(date '+%Y-%m-%d %H:%M:%S') - [4.1] Node_sros2 Test result = 1" >> "$LOG_FILE" 
  echo "1"  
fi

sleep 1
echo "$(date '+%Y-%m-%d %H:%M:%S') - [4.1] Node_sros2 Test finished" >> "$LOG_FILE"
