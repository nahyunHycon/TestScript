...
echo "$(date '+%Y-%m-%d %H:%M:%S') - [1.1] ConfigFile Test started" >> "$LOG_FILE"

sudo chattr +i /home/hyconsoft_rnd/Desktop/TestScript/01_SW_Config/File/requirements.txt
#echo "1234567890" >> /home/hyconsoft_rnd/Desktop/TestScript/01_SW_Config/File/requirements.txt
...
if [ "$md5_checksum" == "$expected_md5" ]; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') - [1.1] ConfigFile Test result = 0 " >> "$LOG_FILE"
    echo "0"    
else
    echo "$(date '+%Y-%m-%d %H:%M:%S') - [1.1] ConfigFile Test result = 1 " >> "$LOG_FILE"
    echo "1"
fi

sleep 1
echo "$(date '+%Y-%m-%d %H:%M:%S') - [1.1] ConfigFile Test finished" >> "$LOG_FILE"
