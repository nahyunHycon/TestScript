...
echo "$(date '+%Y-%m-%d %H:%M:%S') - [5.1] Individual_Information SROS2 Test started" >> "$LOG_FILE"

existing_value1=/home/hyconsoft_rnd/sros2_demo/demo_keystore
existing_value2=true
existing_value3=Enforce

TARGET=/home/hyconsoft_rnd/Desktop/TestScript/05_individual_information/file/config.txt
...
if [ "$existing_value1" == "$extracted_value1" ]&&[ "$existing_value2" == "$extracted_value2" ]&&[ "$existing_value3" == "$extracted_value3" ]; then
  echo "$(date '+%Y-%m-%d %H:%M:%S') - [5.1] Individual_Information SROS2 Test result = 0" >> "$LOG_FILE" 
  echo "0"  
else
  echo "$(date '+%Y-%m-%d %H:%M:%S') - [5.1] Individual_Information SROS2 Test result = 1" >> "$LOG_FILE" 
  echo "1"  
fi

sleep 1
echo "$(date '+%Y-%m-%d %H:%M:%S') - [5.1] Individual_Information SROS2 Test finished" >> "$LOG_FILE"
